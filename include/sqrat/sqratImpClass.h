namespace Sqrat
{
    ...
    class ImprovedAllocator : public DefaultAllocator<C>
    {
        ...
        {
            sq->setinstanceup(vm, 1, instance);
            sq->setreleasehook(vm, 1, &DefaultAllocator<C>::Delete);
            return 0;
        }
    ...
    class ImprovedClass : public Class<C, A>
    {
        ImprovedClass &BindConstructor(SQFUNCTION method, SQInteger nParams)
        {
            ...
            sq->pushobject(vm, classObj);
            sq->pushstring(vm, _SC("constructor"), -1);
            sq->newclosure(vm, method, 0);
            sq->setparamscheck(vm, nParams + 1, NULL);
