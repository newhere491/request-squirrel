...
#define scvprintf vprintf
    ...
    SQInteger SqratVM::runtimeErrorHandler(HSQUIRRELVM v)
    {
        ...
        if (sq->gettop(v) >= 1)
        {
            ...
            if (SQ_SUCCEEDED(sq->getstring(v, 2, &sErr)))
            {
    ...
    SqratVM::SqratVM()
        : m_vm(sq->open(1024)), m_rootTable(new Sqrat::RootTable(m_vm)), m_script(new Sqrat::Script(m_vm)), m_lastErrorMsg()
    {
        s_addVM(m_vm, this);

        // register std libs
        sq->pushroottable(m_vm);
    ...
    {
        sq->setprintfunc(m_vm, printFunc, errFunc);
    }

    void SqratVM::setErrorHandler(SQFUNCTION runErr, SQCOMPILERERROR comErr)
    {
        sq->newclosure(m_vm, runErr, 0);
        sq->seterrorhandler(m_vm);

        sq->setcompilererrorhandler(m_vm, comErr);
    }
